...
def make_plot(rows: list[dict[str, Any]], plot_path: Path) -> None:
    ...
    ax.set_title("Parallel Scaling Efficiency on Capella for SFT on Qwen3-8B")
    ax.set_xticks(range(len(gpus)))
    ax.set_xticklabels(xlabels)
    ax.set_ylim(0, 115)
...
def main() -> None:
    ...
    parser.add_argument(
        "--logs-dir",
        type=Path,
        default=Path("/data/cat/ws/hama901h-Posttraining/.logs/Qwen3-8B/throughput/8B"),
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("./throughput_8B.csv"),
        help="Optional CSV output path.",
    )
    parser.add_argument(
        "--plot",
        type=Path,
        default=Path("./throughput_8B.png"),
        metavar="FILE.png",
