...
show_help() {
  cat <<EOF
Usage: $(basename "$0") [--dry-run|-n] [--dependency=<spec>]  # creates files but does not call sbatch

Options:
  --dry-run, -n         Do not submit jobs; just create files and print the sbatch commands
  --dependency=<spec>   Pass a SLURM --dependency to every submitted job,
                        e.g. --dependency=afterok:1234567
...
for lr in "${LRS[@]}"; do
  for beta in "${BETAS[@]}"; do
    ...
    job_name="DPO-LR${lr}_Beta${beta}-Llama-3.2-1B-SFT-Lr3e-5-AO"
    ...
    sed \
      ...
      -e "s/^beta: .*/beta: ${beta}/" \
      -e "s#^output_dir: .*#output_dir: /data/cat/ws/hama901h-judge-checkpoint/checkpoints/Llama-3.2-1B/SFT-Lr3e-5-AO/${job_name}/#" \
    ...
    if [ "${DRY_RUN}" -eq 1 ]; then
      echo "DRY RUN: would submit: sbatch ${SBATCH_ARGS[*]} ${job_path}  (config: ${config_path})"
    else
      echo "Submitting ${job_path} with ${config_path}${DEPENDENCY:+ (dependency: ${DEPENDENCY})}"
      sbatch "${SBATCH_ARGS[@]}" "${job_path}"
    fi
  done
done
