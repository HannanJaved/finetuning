#!/usr/bin/env python3
"""Batch analysis for Qwen3 pruned configs grouped by search space."""
...
FILENAME_RE = re.compile(
    r"subnet_qwen(?:_8b)?_model_evo_evolutionary_search_qwen_(.+)_([0-2])_100_epochs\.yaml$"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run pruning analysis for Qwen3 evolutionary-search configs.",
    )
    parser.add_argument(
        "--config-dir",
        required=True,
        help="Directory containing subnet_qwen*_model_evo_evolutionary_search_qwen_*.yaml files.",
    )
    parser.add_argument(
        "--original",
        required=True,
    ...
    parser.add_argument(
        "--label",
        default="qwen",
        help="Label prefix for outputs (e.g. qwen3_8b, qwen3_32b).",
    )
...
def _collect_configs(config_dir: str) -> Dict[str, Dict[int, str]]:
    ...
    for entry in Path(config_dir).glob("subnet_qwen*_model_evo_evolutionary_search_qwen_*_100_epochs.yaml"):
...
def main() -> int:
    ...
    if not grouped:
        raise RuntimeError(f"No evolutionary_search configs found in {args.config_dir}")
    ...
    for search_space, buckets in sorted(grouped.items()):
        ...
        labels = [f"{search_space} (bucket {b})" for b in sorted(buckets.keys())]
        ...
        _write_summary_file(output_dir, f"{args.label}_{search_space}", pa.HEADERS, rows, summary_lines)
    ...
    print(f"Saved Qwen3 search-space analyses to: {args.output_root}")
    return 0
