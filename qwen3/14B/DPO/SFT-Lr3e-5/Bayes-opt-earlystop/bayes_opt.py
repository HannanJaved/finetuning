...
def write_config(trial_dir: Path, lr: float, beta: float) -> Path:
    ...
    run_name = f"Qwen3-14B-DPO-BO-EarlyStop-trial{trial_dir.name}-b{beta:.4f}-lr{lr:.2e}"
    out_dir = (
        "/data/horse/ws/hama901h-BFTranslation/checkpoints/Qwen/"
        "Qwen3-14B-Base/SFT-sweep-Early-Stop/"
        f"{run_name}/"
    )
...
def objective(trial: optuna.Trial) -> float:
    ...
    run_name = f"Qwen3-14B-DPO-BO-EarlyStop-t{trial.number:03d}"
...
def main():
    storage = f"sqlite:///{SCRIPT_DIR / 'optuna_qwen_dpo_earlystop.db'}"
    sampler = optuna.samplers.TPESampler(n_startup_trials=5, seed=8)
    study = optuna.create_study(
        study_name="qwen3_14B_dpo_bo_earlystop",
        direction=DIRECTION,
        sampler=sampler,
        storage=storage,
        load_if_exists=True,
