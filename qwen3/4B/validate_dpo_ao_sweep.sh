#!/bin/bash
#SBATCH --job-name=Qwen3-4B-DPO-VAL
#SBATCH --output=/data/horse/ws/hama901h-Post-training/hama901h-Posttraining/.logs/Qwen3/4B/DPO-AO-DDP/SFT-LR3e-5/%x_%A_%a.out
#SBATCH --error=/data/horse/ws/hama901h-Post-training/hama901h-Posttraining/.logs/Qwen3/4B/DPO-AO-DDP/SFT-LR3e-5/%x_%A_%a.err
#SBATCH --nodes=1
...
# Grid: 4 LRs x 4 betas = 16 combinations (array index 0-15)
LRS=(1e-6 2e-6 4e-6 8e-6)
BETAS=(0.01 0.02 0.04 0.08)

LR_IDX=$((SLURM_ARRAY_TASK_ID / 4))
BETA_IDX=$((SLURM_ARRAY_TASK_ID % 4))
LR=${LRS[$LR_IDX]}
BETA=${BETAS[$BETA_IDX]}

export WANDB_NAME="Qwen3-4B-DPO-VAL-LR${LR}-Beta${BETA}"

DPO_AO_DIR="/data/horse/ws/hama901h-Post-training/hama901h-Posttraining/finetuning/qwen3/4B/DPO_AO"
