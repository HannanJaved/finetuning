...
counts from the filename pattern emdpo_tp_<N>g<M>n_*.
...
# logged as a Python dict literal, typically on stderr.
...
def print_table(rows: list[dict[str, Any]]) -> None:
    ...
    if baseline and any(r["gpus"] != 1 and r["samples_per_s"] is not None for r in rows):
        print()
        print(f"{'log_file':42} | {'GPUs':>4} | {'scaling eff':>11} | {'ideal':>6}")
        print("-" * 70)
...
def make_plot(rows: list[dict[str, Any]], plot_path: Path) -> None:
    ...
    ax = axes[0]
    ax.plot(gpus, sps, "o-", color="#2563eb", linewidth=2, markersize=7, label="Measured")
    ideal_sps = [baseline_sps * g for g in gpus]
    ax.plot(gpus, ideal_sps, "--", color="#9ca3af", linewidth=1.5, label="Ideal linear")
    ax.set_xlabel("Number of GPUs")
    ax.set_ylabel("Samples / second")
    ax.set_title("EM-DPO Throughput Scaling\n(Qwen3-1.7B, DPO fine-tuning)")
    ax.legend()
    ax.set_xticks(gpus)
    ...
    ax.axhline(100, color="#9ca3af", linestyle="--", linewidth=1.5, label="Ideal (100%)")
    ax.set_xlabel("Number of GPUs")
    ...
    fig.suptitle(
        "Compute Grant Justification — EM-DPO Throughput Sweep",
