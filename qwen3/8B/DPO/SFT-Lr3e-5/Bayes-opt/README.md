# Bayesian Optimization (DPO) – SLURM runner
...
- Runs `bayes_opt.py` for 25 trials (Optuna TPE sampler).
- Each trial writes its own config/job script and submits with `sbatch`.
- Uses **1% validation split** from the training dataset (`dataset_test_split_size: 0.01`).
