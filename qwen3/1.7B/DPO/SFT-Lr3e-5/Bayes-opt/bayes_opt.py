...
def write_config(trial_dir: Path, lr: float, beta: float) -> Path:
    ...
    run_name = f"Qwen3-1.7B-DPO-BO-trial{trial_dir.name}-b{beta:.4f}-lr{lr:.2e}"
    out_dir = f"/data/horse/ws/hama901h-BFTranslation/checkpoints/Qwen/Qwen3-1.7B-Base/SFT-sweep/{run_name}/"
...
def objective(trial: optuna.Trial) -> float:
    ...
    run_name = f"Qwen3-1.7B-DPO-BO-t{trial.number:03d}"
