...
BASE_DIR="/data/cat/ws/hama901h-Posttraining/finetuning/qwen3/1.7B"
...
for lr in "${LRS[@]}"; do
  job_name="Qwen3-1.7B-SFT-LR${lr}"
  config_path="${BASE_DIR}/config_olmo3_sft_${lr}.yaml"
  job_path="${BASE_DIR}/sft_olmo3_${lr}.sh"

  sed \
    -e "s/^learning_rate: .*/learning_rate: ${lr}/" \
    -e "s#^output_dir: .*#output_dir: /data/horse/ws/hama901h-BFTranslation/checkpoints/Qwen/Qwen3-1.7B-Base/SFT-sweep/${job_name}/#" \
