#!/bin/bash
#SBATCH --job-name=Qwen3-Entropy-FirstToken
...
RUN_DIR=${RUN_DIR:?
"Set RUN_DIR to the SFT run directory (Qwen3-*-SFT-LR*)"}
VLLM_TP_SIZE=${VLLM_TP_SIZE:-4}
VLLM_MAX_MODEL_LEN=${VLLM_MAX_MODEL_LEN:-4096}
VLLM_GPU_MEM_UTIL=${VLLM_GPU_MEM_UTIL:-0.9}
VLLM_MAX_NUM_SEQS=${VLLM_MAX_NUM_SEQS:-256}
RUN_NAME=${RUN_NAME:-$(basename "$RUN_DIR")}
RESULT_BASE=${RESULT_BASE:-"/data/horse/ws/hama901h-BFTranslation/finetuning/qwen3/entropy/results_first_token"}
...
python /data/horse/ws/hama901h-BFTranslation/finetuning/qwen3/entropy/compute_entropy_first_token.py \
