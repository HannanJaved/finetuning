#!/bin/bash
#SBATCH --job-name=Gemma3-4B-DPO-SMOKETEST
#SBATCH --output=/data/horse/ws/hama901h-Post-training/hama901h-Posttraining/.logs/Gemma3/4B/DPO/smoketest/%x_%j.out
#SBATCH --error=/data/horse/ws/hama901h-Post-training/hama901h-Posttraining/.logs/Gemma3/4B/DPO/smoketest/%x_%j.err
#SBATCH --nodes=2
...
# No W&B run for the smoke test — report_to: [] is set in the config too,
# this just keeps the env consistent with the real sweep scripts.
export WANDB_PROJECT=instruction-tuning
export WANDB_ENTITY=openeurollm-project
export WANDB_NAME=Gemma3-4B-DPO-SMOKETEST
...
CONFIG_FILE=/data/horse/ws/hama901h-Post-training/hama901h-Posttraining/finetuning/gemma3/4B/DPO/dpo_smoketest_zero3.yaml
