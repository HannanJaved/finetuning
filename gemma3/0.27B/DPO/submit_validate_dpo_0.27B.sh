...
# of the 0.27B model, then optionally submit them.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_TEMPLATE="${SCRIPT_DIR}/validate_dpo_template_0.27B.yaml"

CHECKPOINT_BASE="/data/horse/ws/hama901h-BFTranslation/checkpoints/Gemma3/gemma-3-270m/SFT-sweep/gemma-3-270m-SFT-LR3e-5/DPO"
LOG_DIR="/data/horse/ws/hama901h-Post-training/hama901h-Posttraining/.logs/Gemma3/0.27B/DPO/validate"
...
echo "Generating validation jobs for Gemma3-0.27B (16 configs)"
...
for beta in "${BETAS[@]}"; do
  for lr in "${LRS[@]}"; do
    job_name="Val-Gemma3-0.27B-DPO-Beta${beta}_LR${lr}"
    checkpoint_dir="${CHECKPOINT_BASE}/Beta${beta}_LR${lr}"
    config_path="${SCRIPT_DIR}/validate_dpo_beta${beta}_LR${lr}_0.27B.yaml"
